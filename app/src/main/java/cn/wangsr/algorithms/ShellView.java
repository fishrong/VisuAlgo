...
public class ShellView extends View {
    ...
    public static void show(ShellView myView, int min, int currentIndex, int finished, Handler handler){
        myView.min = min;
        myView.currentIndex = currentIndex;
        myView.finished = finished;

        handler.sendEmptyMessage(123);
        if (ShellActivity.runAuto) {
            try {
                Thread.sleep(800);
